 ...
 *
 * 主要的流程:
 * 配置HttpClientBuilder的连接池管理，cookie，User-Agent，Socket链接控制，重定向处理
 * 设置好了之后就可以生成一个具体的HttpClient实例
 * 接下来是构造请求，为RequestBuilder配置链接地址，请求的方法，请求的数据，请求的header，链接的配置等再用RequestBuilder生成一个具体的HttpUriRequest
 * 最后使用HttpClient执行创建的请求
 */

public class HttpSSLClientDemo3 {
    ...
    //    try {
    //        return new SSLConnectionSocketFactory(createIgnoreVersifySSL());  //跳过ssl验证的
    //    } catch (Exception e){
    //        e.printStackTrace();
    //    }
    ...
    //
    //    //连接池的相关配置
    //    SocketConfig config = SocketConfig.custom().setSoTimeout(3000).setSoKeepAlive(true).setTcpNoDelay(true).build();
    //    manager.setDefaultSocketConfig(config);
    //    manager.setDefaultMaxPerRoute(1000);   //控制最大连接数
    //
    //    httpClientBuilder.setConnectionManager(manager);  //为连接池配置管链接理器
    //    httpClientBuilder.setUserAgent(useragent); //配置user-agent欺骗服务器
    //    //为请求添加压缩属性
    ...
    //    ));
    //
    //    //配置cookie为生成的http请求都添加这个cookie，也是为了欺骗服务器
    ...
    //    RequestBuilder requestBuilder = selectRequestMethod(methods,new HashMap<>());
